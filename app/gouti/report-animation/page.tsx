/**
 * Report animation for the gouti journey.
 * Edit freely here. Locked template: app/gouti/report-preview/page.tsx
 */
"use client";

import { Suspense } from "react";
import { useSearchParams } from "next/navigation";
import { Inter } from "next/font/google";
import Image from "next/image";
import Link from "next/link";
import type { ReactNode } from "react";
import ReportBuildIn from "@/components/gouti/ReportBuildIn";
import {
  previewReportData,
  type ReportData,
} from "@/lib/report-preview-data";
import { isUnlockReady } from "@/lib/unlock-ready";

export type { ReportData };

const inter = Inter({ subsets: ["latin"] });

function ReportHeading({ children }: { children: ReactNode }) {
  return (
    <h2 className="m-0 text-[52px] font-bold leading-[63px] tracking-[-0.04em] text-[#e48217]">
      ✰ {children}
    </h2>
  );
}

function ReportBrutalButton({
  children,
  className = "",
  twoLine,
  href,
  buildStep,
}: {
  children?: ReactNode;
  className?: string;
  twoLine?: { primary: string; secondary: string };
  href?: string;
  buildStep?: number;
}) {
  const sharedClassName = `inline-flex items-center justify-center border-2 border-black bg-[#ffc940] font-semibold text-black shadow-[4px_4px_0_0_#000] transition hover:bg-[#ffd966] ${className}`;
  const buildAttr =
    buildStep != null
      ? { "data-report-build": String(buildStep) }
      : undefined;

  const content = twoLine ? (
    <>
      <span className="block text-[32px] font-bold leading-[35px]">
        {twoLine.primary}
      </span>
      <span className="block text-[20px] leading-[20px] font-semibold">
        {twoLine.secondary}
      </span>
    </>
  ) : (
    children
  );

  if (href) {
    const isAbsolute = /^https?:\/\//i.test(href);

    if (isAbsolute) {
      return (
        <a href={href} className={sharedClassName} {...buildAttr}>
          {content}
        </a>
      );
    }

    return (
      <Link href={href} className={sharedClassName} {...buildAttr}>
        {content}
      </Link>
    );
  }

  return (
    <button type="button" className={sharedClassName} {...buildAttr}>
      {content}
    </button>
  );
}

function ReportCard({
  title,
  items,
  description,
}: {
  title: string;
  items: string[];
  description: string;
}) {
  return (
    <section className="h-[669px] w-[966px] border border-black bg-white px-[39px] py-[47px]">
      <ReportHeading>{title}</ReportHeading>
      <ol className="mb-[28px] mt-[29px] list-decimal space-y-[22px] pl-[48px] text-[40px] leading-[49px] tracking-[-0.01em]">
        {items.slice(0, 3).map((item) => (
          <li key={item}>{item}</li>
        ))}
      </ol>
      <p className="m-0 max-h-[250px] overflow-hidden text-[40px] leading-[50px] tracking-[-0.01em]">
        {description}
      </p>
    </section>
  );
}

export function ReportTemplate({
  data,
  gameplanHref = "/gouti/landing",
  exportMode = false,
  instantReady = false,
}: {
  data: ReportData;
  gameplanHref?: string;
  exportMode?: boolean;
  instantReady?: boolean;
}) {
  return (
    <main
      className={`report-page-main ${inter.className} ${exportMode ? "bg-white" : "min-h-screen overflow-x-hidden bg-transparent pb-3 sm:pb-6 lg:pb-10"}`}
    >
      {!exportMode ? (
        <ReportBuildIn gentleScroll instantReady={instantReady} />
      ) : null}
      <div
        data-report-camera
        className={
          exportMode
            ? "report-viewport h-[3365px] w-[2214px]"
            : `report-viewport mx-auto -mt-[11px] h-[494px] w-full origin-top-left overflow-hidden md:-mt-[21px] md:h-[943px] xl:-mt-[34px] xl:h-[1496px]${instantReady ? "" : " opacity-0"}`
        }
      >
        <article
          data-report-article
          className={
            exportMode
              ? "relative h-[3365px] w-[2214px] bg-white text-black"
              : "relative h-[3525px] w-[2356px] origin-top-left scale-[0.1400679] bg-transparent text-black md:scale-[0.2674024] xl:scale-[0.4244482]"
          }
        >
          <div
            id="report-pdf-source"
            data-report-frame
            className={
              exportMode
                ? "relative h-[3365px] w-[2214px] bg-white"
                : "absolute left-0 top-[80px] h-[3365px] w-[2214px] overflow-hidden rounded-[25px] border-[0.3px] border-black bg-white shadow-[0_4px_4px_rgba(0,0,0,0.25)]"
            }
          >
            <div data-report-content className="relative h-full w-full">
            {exportMode ? (
              <img
                src="/logo-dp.svg"
                alt="Your Blueprint"
                width={168}
                height={168}
                data-report-build="2"
                className="absolute right-[120px] top-[96px] h-[168px] w-[168px]"
              />
            ) : (
              <Image
                src="/logo-dp.svg"
                alt="Your Blueprint"
                width={168}
                height={168}
                data-report-build="2"
                className="absolute right-[120px] top-[96px] h-[168px] w-[168px]"
              />
            )}
            <header
              data-report-build="1"
              className="absolute left-[121px] right-[109px] top-[96px] h-[254px] border-b border-black"
            >
              <p className="m-0 text-[72px] font-bold leading-[87px] tracking-[-0.04em]">
                Personalized Creator Blueprint
              </p>
              <h1 className="m-[6px_0_0] whitespace-nowrap text-[100px] font-bold leading-[121px] tracking-[-0.04em] text-[#e48217]">
                {data.name.toUpperCase()}
              </h1>
            </header>

            <section
              data-report-profile
              className="absolute left-[102px] top-[420px] h-[901px] w-[2001px] border-b border-black"
            >
              <div
                data-report-photo-group
                className="absolute left-0 top-0 h-[556px] w-[551px]"
              >
                <div
                  data-report-photo-bg
                  className="absolute left-0 top-0 h-[556px] w-[516px] rotate-[-2deg] rounded-[30px] bg-[#d9d9d9]"
                />
                {exportMode ? (
                  <img
                    src={data.profileImage}
                    alt={data.name}
                    width={518}
                    height={510}
                    data-report-photo
                    className="absolute left-[33px] top-[14px] h-[510px] w-[518px] rounded-[30px] object-cover"
                  />
                ) : (
                  <Image
                    src={data.profileImage}
                    alt={data.name}
                    width={518}
                    height={510}
                    unoptimized
                    data-report-photo
                    className="absolute left-[33px] top-[14px] h-[510px] w-[518px] rounded-[30px] object-cover"
                  />
                )}
              </div>
              <div className="absolute left-[683px] top-[37px] w-[1305px]">
                <div
                  data-report-info
                  className="relative grid grid-cols-[1fr_1.35fr] gap-y-[27px] text-[48px] leading-[58px]"
                >
                  <p
                    data-report-type-line
                    data-skel-w="78%"
                    className="col-span-2 m-0 whitespace-nowrap"
                  >
                    <b>NAME:</b> {data.name}
                  </p>
                  <p
                    data-report-type-line
                    data-skel-w="42%"
                    className="m-0"
                  >
                    <b>AGE:</b> {data.age}
                  </p>
                  <p
                    data-report-type-line
                    data-skel-w="88%"
                    className="m-0 whitespace-nowrap"
                  >
                    <b>BASED:</b> {data.location}
                  </p>
                  <p
                    data-report-type-line
                    data-skel-w="92%"
                    className="col-span-2 m-0"
                  >
                    <b>GOAL:</b>{" "}
                    {data.goal.includes("help me") ? (
                      <>
                        {data.goal.split("help me")[0]}help me
                        <br />
                        {data.goal.split("help me")[1]?.trim()}
                      </>
                    ) : (
                      data.goal
                    )}
                  </p>
                </div>
                <h2
                  data-report-type-line
                  data-skel-w="68%"
                  className="mb-0 mt-[57px] text-[72px] font-bold leading-[87px] tracking-[-0.04em]"
                >
                  Your Creator Identity:
                </h2>
                <p
                  data-report-type-line
                  data-skel-w="82%"
                  className="m-0 text-[64px] font-bold leading-[77px] tracking-[-0.04em] text-[#e48217]"
                >
                  {data.creatorIdentity}
                </p>
              </div>
              <p
                data-report-build="6"
                className="absolute left-0 top-[657px] m-0 w-full text-[48px] leading-[62px] tracking-[-0.01em]"
              >
                {data.identityDescription}
              </p>
            </section>

            <section
              data-report-build="7"
              className="absolute left-[104px] top-[1387px] w-[2001px] pb-[43px]"
            >
              <ReportHeading>Why This Direction Fits You</ReportHeading>
              <ul className="m-[29px_0_28px] grid list-none grid-cols-2 gap-y-[23px] p-0 text-[44px] leading-[53px] tracking-[-0.01em]">
                {data.whyItFits.slice(0, 4).map((item) => (
                  <li key={item}>
                    <span className="mr-[17px] text-[#ffb553]">✓</span>
                    {item}
                  </li>
                ))}
                {data.whyItFits[4] && (
                  <li className="col-span-2" key={data.whyItFits[4]}>
                    <span className="mr-[17px] text-[#ffb553]">✓</span>
                    {data.whyItFits[4]}
                  </li>
                )}
              </ul>
              <p className="m-0 mt-[24px] text-[48px] leading-[62px] tracking-[-0.01em]">
                {data.whyItFitsDescription}
              </p>
            </section>

            <div
              data-report-build="8"
              className="absolute left-[104px] top-[1967px] flex gap-[73px]"
            >
              <ReportCard
                title="Strengths You Already Have"
                items={data.strengths}
                description={data.strengthsDescription}
              />
              <ReportCard
                title="Things Holding You Back"
                items={data.blockers}
                description={data.blockersDescription}
              />
            </div>

            <section className="absolute left-[104px] top-[2747px] grid w-[2001px] grid-cols-2 gap-[73px]">
              <div data-report-build="9">
                <ReportHeading>Your Next Move?</ReportHeading>
                <ul className="m-[30px_0_0] list-none space-y-[23px] p-0 text-[44px] leading-[53px] tracking-[-0.01em]">
                  {data.nextMoves.slice(0, 6).map((move, index) => (
                    <li key={`${move}-${index}`}>- {move}</li>
                  ))}
                </ul>
              </div>
              <div className="flex min-h-[420px] flex-col">
                <div data-report-build="9">
                  <ReportHeading>What&apos;s still missing?</ReportHeading>
                  <p className="m-[29px_0_0] line-clamp-3 whitespace-normal text-[44px] leading-[53px] tracking-[-0.01em]">
                    {[
                      data.gameplanCopyLine1,
                      data.gameplanCopyLine2,
                      data.gameplanCopyLine3,
                    ]
                      .join(" ")
                      .replace(/\s+/g, " ")
                      .trim()}
                  </p>
                  <p className="m-[53px_0_0] whitespace-nowrap text-[44px] leading-[53px] tracking-[-0.01em]">
                    let&apos;s build the plan to get you there.
                  </p>
                </div>
                <ReportBrutalButton
                  href={gameplanHref}
                  buildStep={10}
                  className="mt-[24px] h-[98px] w-full text-[48px] font-bold leading-[58px]"
                >
                  Checkout your Gameplan →
                </ReportBrutalButton>
              </div>
            </section>
            </div>
          </div>
        </article>
      </div>
    </main>
  );
}

export default function ReportAnimationPage() {
  return (
    <Suspense fallback={null}>
      <ReportAnimationPageInner />
    </Suspense>
  );
}

function ReportAnimationPageInner() {
  const searchParams = useSearchParams();
  const raw = searchParams.get("n")?.trim();
  const name =
    raw && raw.length > 0 ? raw : previewReportData.name;

  return (
    <ReportTemplate
      data={{ ...previewReportData, name }}
      instantReady={isUnlockReady(searchParams)}
    />
  );
}
